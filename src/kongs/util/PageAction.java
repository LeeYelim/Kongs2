...
public class PageAction {
	 ...
	 * */
   public static int pageSize = 10  ;//한페이지에 보여질 게시물 수
   public static int blockCount = 3 ;//한페이지당 보여질 []의 개수
   ...
   /**
    * 페지분할의 결과를 String으로 만들어 리턴해 줄 메소드 작성
    * ex) [이전]   [1][2][3] .. [다음]
    * 
    * */
   public static String pageNumber(){
	   ...
	   //2. [이전] 만들기
	    ...
	    if( (startPage - blockCount) >  0 ){
	       str="<a href='front?command=contest&pageNum="+(startPage-1)+"&keyField="+keyField+"&keyWord="+keyWord+"'>◀이전 </a>&nbsp;&nbsp;" ;
	    }
	    ...
	    for(int i = startPage ; i < (startPage+blockCount) ; i++){
	    	if(i==pageNum)
	           str+="<font color='red'>["+i +"]</font>&nbsp;";
	    		...
	    		str+="<a href='front?command=contest&pageNum="+i+"&keyField="+keyField+"&keyWord="+keyWord+"'>["+i +"]</a>&nbsp;";	
	   ...
	   //4.[다음]  만들기
	    ...
	    if( (startPage + blockCount) <= pageCount ){
	       str+="<a href='front?command=contest&pageNum="+(startPage+blockCount)+"&keyField="+keyField+"&keyWord="+keyWord+"'> ▶다음 </a>" ;
	    }   
